...
\noindent Here $\mathsf{fund}$ identifies the funding anchor, $\mathsf{assets}$ commits to the asset registry, $\mathsf{descriptor}$ commits to settlement-output derivation, and $\mathsf{challenge}$ specifies the relative finalisation policy.
...
A vault Cell is admissible only if its lock or type script rejects every spend that is not authorised by the currently valid state evidence and the committed settlement descriptor.
\end{definition}
...
Sponsor funds are not part of the channel-owned value partition. They are ordinary wallet Cells, watchtower-owned Cells, or pre-authorised fee budgets whose authorisation is limited by channel identity, state-number range, maximum fee, expiry, allowed sponsor source, and change destination.
...
\node[box, fill=MorphGreen!9, draw=MorphGreen!75!black] at (7.5,0.9) (budget) {Sponsor Budget\\fee bound, expiry};
...
verify_state_transition(tx):
  ...
  require new_header.settlement_descriptor_commitment
          == old_header.settlement_descriptor_commitment
  require new_header.descriptor_version
...
verify_vault_spend(tx):
  ...
  if op == FINALIZE:
    ...
    require state_input_since_satisfies(header.challenge_policy)
...
verify_sponsor_budget_spend(tx):
  ...
  require current_time_or_block <= policy.expiry
...
\textsf{FINALIZE} consumes the current settling State Cell and the relevant vault Cells after the relative \texttt{since} delay has matured. It creates the settlement outputs authorised by the committed settlement descriptor. Cooperative close is a special path that carries explicit authorisation from the required participants and does not require a challenge delay.
...
A factory witness contains an access manifest, a proof bundle, touched leaf payloads, and the required signatures:
\begin{lstlisting}
FactoryWitness {
  state_header
  access_manifest
...
If a factory action cannot prove non-interference, then a conservative first implementation should require signatures from all factory participants. If non-interference is proven, the signing set may be reduced to the affected participants.
\end{proposition}

\begin{proof}
Without non-interference, untouched leaves may still depend on shared membership, reserve, or exit-right state. A local signature set would not authorise those possible effects. Full-participant signatures authorise global effects; alternatively, a non-interference proof removes those effects from the transaction semantics.
\end{proof}
...
The terms respectively represent detection confirmations, watchtower polling interval, transaction construction and propagation, confirmation of the newer-state transaction, and a margin for fee bumping and reorganisation risk. Implementations may choose block-number, epoch, or timestamp based \texttt{since}, but must test against current CKB hardfork semantics~\cite{ckb-since,ckb-since-timestamp,ckb-since-epoch}.
...
The sponsor policy must be bounded by channel identity, state-number range, maximum fee, expiry, allowed sponsor source, and change destination. The watchtower should not be able to redirect channel-owned value or drain arbitrary wallet funds.

\section{Safety Properties}

\begin{proposition}[Funding Anchor Uniqueness]
If the State Header binds a funding anchor identity and scripts verify that referenced \texttt{cell\_deps} match that identity, then a descriptor-shaped but non-authoritative Cell cannot serve as the channel's funding anchor.
\end{proposition}

\begin{proof}
The script compares the referenced Cell identity with the identity committed in the State Header. A different Cell, even with similar data, has a different outpoint or genesis identity and is rejected.
\end{proof}

\begin{proposition}[Vault Spend Safety]
If each vault lock accepts only current-state-authorised settlement or materialisation transactions, then channel-owned value cannot be spent through ordinary wallet paths.
\end{proposition}

\begin{proof}
Every spend of channel-owned value must satisfy the vault lock. By assumption, the lock rejects transactions lacking current state evidence and descriptor-admissible outputs. Ordinary wallet signatures alone are insufficient.
\end{proof}
...
\begin{enumerate}[label=E\arabic*.]
  ...
  \item sponsor-policy failure cases, including insufficient budget, expired policy, and wrong change destination;
...
\begin{enumerate}[label=O\arabic*.]
  ...
  \item Different xUDT implementations may impose different constraints.
  \item The paper now specifies script-level semantics, but not byte-level Molecule schemas or complete CKB-VM implementations.
