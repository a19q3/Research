...
funding anchor, state number, signing digest, source State outpoint, and
descriptor commitment/version metadata. It validates the signatures and metadata
for the plaintext bilateral profile. Broader packages that carry explicit
generic asset-registry bytes, challenge-policy bytes, or commitment-only proof
material remain representation-profile work.
...
S5 & Splice advances funding context without changing channel identity; enforced by signed funding epoch and vault-set commitment & post-splice state binds new anchor and vault set & pre-splice package controls post-splice vaults \\
\bottomrule
\end{tabular}
